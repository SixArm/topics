...
class CynefinFramework {
  ...
  initializeDomains() {
    ...
    domains.set(
      "simple",
      new CynefinDomain("Simple/Clear", {
        traits: [
          "predictable",
          "repeatable",
          "wellUnderstood",
    ...
    domains.set(
      "complicated",
      new CynefinDomain("Complicated", {
        traits: [
          "analyzable",
          "requiresExpertise",
          "multipleSolutions",
          "causeEffectDiscoverable",
        ],
...
class TestingStrategyAdvisor {
  ...
  /**
   * Recommends digital health level
   * @param {string} domain - Cynefin domain
...
["Simple", "Complicated", "Complex", "Chaotic", "Disorder"].forEach(
  (domain) => {
    ...
    console.log(
      `  Approaches: ${strategy.approaches.slice(0, 2).join(", ")}...`,
    );
  },
);
