...
class TestingLevel {
  static levels = {
    ...
    integration: {
      ...
      examples: [
        "API endpoints communicate correctly",
...
class TestSuite {
  ...
  execute() {
    ...
    results.passRate =
      ((results.passed / this.testCases.length) * 100).toFixed(1) + "%";
...
class TestingStrategy {
  ...
  static getTestingPyramid() {
    return {
      description:
        "Testing pyramid recommends more unit tests than integration tests, and more integration tests than E2E tests",
      levels: [
...
suite.addTestCase(
  new TestCase({ title: "Invalid password", priority: "high" }),
);
suite.addTestCase(
  new TestCase({ title: "Forgot password", priority: "medium" }),
);
suite.addTestCase(new TestCase({ title: "Remember me", priority: "low" }));
suite.addTestCase(
  new TestCase({ title: "Session timeout", priority: "medium" }),
);
...
pyramid.levels.forEach((level) => {
  console.log(
    `${level.level}: ${level.percentage} (${level.speed}, ${level.cost} cost)`,
  );
});
